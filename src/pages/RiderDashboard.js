...
import { requestRide, fetchNearbyDrivers } from '../api'; // This line is now present as requested
...
export default function RiderDashboard() {
  ...
  const handleRequestRide = async () => {
    ...
    try {
      ...
      if (
        res.dropoff_coords &&
        typeof res.dropoff_coords.lng === 'number' &&
        typeof res.dropoff_coords.lat === 'number'
      ) {
        directionsRef.current.setDestination([res.dropoff_coords.lng, res.dropoff_coords.lat]);
      } else {
        alert('Invalid dropoff coordinates received from server.');
      }
    } catch (err) {
      console.error(err);
